...
# -*- coding: utf-8 -*-
"""
wi.dat と snow.dat から Leaflet Web地図（wi.html）を生成するスクリプト

機能:
  - WIの値に応じて色分けした3次メッシュセルを表示（カスタムCanvasレイヤー）
  - 年最深積雪を色分けした3次メッシュセルを表示（カスタムCanvasレイヤー）
  - クリックでその地点のWI値・積雪深をポップアップ表示
  - Google Hybrid（写真）と地理院地図（標準地図）をベースレイヤーとして追加
  - レイヤーコントロールによる表示切り替え

パフォーマンス最適化:
  - L.rectangle ではなくカスタムCanvasレイヤーを使用
  - JSオブジェクト生成を避けてcanvasに直接描画
  - クリックイベントはヒットテストで処理
"""
...
HTML_FILE = os.path.join(os.path.dirname(__file__), 'wi.html')

# 3次メッシュのセルサイズ（度）
CELL_HEIGHT = 1.0 / 120.0  # 緯度方向（約830m）
CELL_WIDTH  = 1.0 / 80.0   # 経度方向（約1125m）


def load_wi_data():
    """wi.dat を読み込んでデータリストを返す"""
    records = []
    with open(WI_FILE, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if len(parts) == 4:
                ...
                lat = float(parts[2])
                wi  = float(parts[3])
                records.append([round(lat, 6), round(lon, 6), round(wi, 1)])
    return records


def load_snow_data():
    """snow.dat を読み込んでデータリストを返す（積雪 0 cm の地点は除く）"""
    records = []
    with open(SNOW_FILE, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if len(parts) == 4:
                ...
                if snow > 0:  # 0 cm（無積雪地点）は表示しない
                    records.append([round(lat, 6), round(lon, 6), snow])
    return records


def create_html(wi_records, snow_records):
    """Leaflet Web地図のHTMLを生成する（カスタムCanvasレイヤー版）"""

    wi_json   = json.dumps(wi_records,   separators=(',', ':'))
    snow_json = json.dumps(snow_records, separators=(',', ':'))
...
<head>
  ...
  <title>気候指数 WebGIS – 暖かさの指数・年最深積雪</title>
  ...
  <style>
    ...
    .info-panel {{
      ...
      border-radius: 6px; box-shadow: 0 1px 5px rgba(0,0,0,0.4);
      font-size: 12px; max-width: 220px; line-height: 1.5;
    }}
...
<body>
  ...
  <script>
    ...
    // 3次メッシュのセルサイズ（度）
    const CELL_H = {CELL_HEIGHT:.10f};  // 緯度方向
    const CELL_W = {CELL_WIDTH:.10f};   // 経度方向
    ...
    function getSnowColor(snow) {{
      if (snow <= 0) return null;  // 0 cm は描画しない
    ...
    // =====================================================================
    // 地図の初期化（URLハッシュ #zoom/lat/lon があればその位置を使用）
    // =====================================================================
    ...
    const MeshCanvasLayer = L.Layer.extend({{
      ...
      onAdd: function(map) {{
        ...
        this._canvas.style.pointerEvents = 'none';  // クリックを地図に透過
      ...
      _redraw: function() {{
        ...
        // -------------------------------------------------------
        // キャンバスをビューポート原点 (0,0) に合わせる。
        // LeafletはパンするときmapPane全体をCSSのtransformで
        // オフセットする。canvasはoverlayPane（mapPaneの子）内に
        // あるため同じだけ平行移動してしまう。
        // これを打ち消すために canvas を -mapPanePos だけ移動し、
        // canvas の (0,0) がコンテナの (0,0) と一致するようにする。
        // -------------------------------------------------------
        ...
        for (let i = 0; i < data.length; i++) {{
          ...
          if (lat + CELL_H < sw.lat || lat - CELL_H > ne.lat ||
              lon + CELL_W < sw.lng || lon - CELL_W > ne.lng) {{
            continue;
          }}

          const color = colorFn(val);
          if (!color) continue;  // null = 描画しない（0 cm 積雪など）
          ...
          const x = Math.round(ptSW.x);
          const y = Math.round(ptNE.y);
          const w = Math.max(1, Math.round(ptNE.x - ptSW.x));
          const h = Math.max(1, Math.round(ptSW.y - ptNE.y));
      ...
      hitTest: function(latlng) {{
        ...
        for (let i = 0; i < data.length; i++) {{
          const dlat = data[i][0];
          const dlon = data[i][1];
          if (Math.abs(dlat - lat) <= CELL_H / 2 &&
              Math.abs(dlon - lon) <= CELL_W / 2) {{
            return data[i];
          }}
        }}
        return null;
    ...
    // =====================================================================
    // 汎用 ToggleLayer（レイヤーコントロール用ラッパー）
    // =====================================================================
    const ToggleLayer = L.Layer.extend({{
      initialize: function(meshLayer) {{
        this._meshLayer = meshLayer;
      }},
      onAdd: function() {{
        this._meshLayer.setVisible(true);
        this._meshLayer._redraw();
      }},
      onRemove: function() {{
        this._meshLayer.setVisible(false);
      }}
    }});

    // =====================================================================
    // レイヤーインスタンスを作成して地図に追加
    // =====================================================================
    ...
    snowLayer.addTo(map);
    snowLayer.setVisible(false);  // デフォルト非表示

    // ToggleLayer: WI はデフォルト表示、積雪はデフォルト非表示
    const wiToggle = new ToggleLayer(wiLayer);
    wiToggle.addTo(map);

    const snowToggle = new ToggleLayer(snowLayer);
    // addTo(map) しない → デフォルト非表示

    // =====================================================================
    // クリックイベント: WI値・積雪深をポップアップ表示
    // =====================================================================
    map.on('click', function(e) {{
      ...
      if (coordHit) {{
        ...
        L.popup()
          .setLatLng(e.latlng)
          .setContent(
            parts.join('<hr style="margin:4px 0;">') +
            '<br><small>' + coord + '</small>'
          )
          .openOn(map);
      }}
    }});
    ...
    // WI 凡例（右下）
    // =====================================================================
    const wiLegend = L.control({{ position: 'bottomright' }});
    wiLegend.onAdd = function(map) {{
      const div = L.DomUtil.create('div', 'legend');
      let html = '<h4>暖かさの指数（WI）</h4>';
      for (let i = 0; i < WI_COLORS.length; i++) {{
        html += `<div class="legend-item">
          <div class="legend-color" style="background:${{WI_COLORS[i]}};"></div>
          <span>${{WI_LABELS[i]}}</span></div>`;
      }}
    ...
    // 積雪凡例（左下）
    // =====================================================================
    const snowLegend = L.control({{ position: 'bottomleft' }});
    snowLegend.onAdd = function(map) {{
      const div = L.DomUtil.create('div', 'legend');
      let html = '<h4>年最深積雪</h4>';
      for (let i = 0; i < SNOW_COLORS.length; i++) {{
        html += `<div class="legend-item">
          <div class="legend-color" style="background:${{SNOW_COLORS[i]}};"></div>
          <span>${{SNOW_LABELS[i]}}</span></div>`;
      }}
      html += '<hr style="margin:5px 0;"><small>出典: 国土数値情報 G02-22<br>（1991–2020年平年値）<br>欠損・0 cm 地点は非表示</small>';
      div.innerHTML = html;
      return div;
    }};
    snowLegend.addTo(map);
    ...
    // =====================================================================
    // URL ハッシュを地図の移動に合わせて自動更新
    // （ブラウザのアドレスバーをそのままコピーしても共有できる）
    // =====================================================================
    map.on('moveend', function() {{
      const c    = map.getCenter();
      const zoom = map.getZoom();
      history.replaceState(null, '',
        '#' + zoom + '/' + c.lat.toFixed(5) + '/' + c.lng.toFixed(5));
    }});

    // =====================================================================
    // 共有ボタン（左上）: 現在のURL をクリップボードにコピー
    // =====================================================================
    ...
    const ShareControl = L.Control.extend({{
      ...
      onAdd: function(map) {{
        ...
        L.DomEvent.on(btn, 'click', function(e) {{
          ...
          // moveend がまだ発火していない場合に備えて現在位置でハッシュを更新
          const c    = map.getCenter();
          const zoom = map.getZoom();
...
def main():
    ...
    print(f"  {len(wi_records)} 件のWIデータを読み込みました。")

    print("snow.dat を読み込み中...")
    snow_records = load_snow_data()
    print(f"  {len(snow_records)} 件の積雪データを読み込みました（0 cm 除く）。")

    print("wi.html を生成中...")
    html = create_html(wi_records, snow_records)

    with open(HTML_FILE, 'w', encoding='utf-8') as f:
        f.write(html)

    file_size_mb = os.path.getsize(HTML_FILE) / (1024 * 1024)
    print(f"wi.html を作成しました: {HTML_FILE}")
